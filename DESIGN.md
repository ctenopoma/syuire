...
- 既存の朱は縦線と番号で表示し、タップで引用・返信・状態を確認する。
  朱の対象となった本文(anchor)は本文中で強調表示する。対象が見つからない朱は番号だけを示す。
- 見出しの目次と朱の一覧を側面パネルに出し、本文の該当箇所へ移動できるようにする。
- フェンス付きコードは言語ごとに着色する。着色は表示文字列とソースの対応を変えない(既存の run を分割するだけ)。
- 接続画面は「リポジトリ」と「PAT」の 2 項目で始められるようにする。リポジトリは GitHub の URL や `owner/repo` を受け付け、
  ブランチは既定ブランチを、署名は PAT のログイン名を既定にする。対象ファイルは接続後の一覧で選ぶ。
- ヘッダにはリポジトリ、ブランチ、ファイル、未保存件数、保存先、同期状態を表示する。
- 原稿の raw HTML は実行しない。コメントは文字列として表示し、リンクのスキームも制限する。
  外部画像は自動取得せず、リポジトリ内画像は選択した版から取得する(既定で自動、表示設定で手動に切替可)。
...
| 最後に開いた接続先・ブランチ・パス、最近の接続先と各接続で開いたファイルの短い一覧、表示設定(文字サイズ・配色等) | `localStorage`。端末内の非秘密情報。利用者が消去できる |
...
- Web の接続画面(2026-09-13 見直し): ブランチ欄が空なら GitHub の既定ブランチに接続する。署名(author)が空なら PAT のログイン名を使う。
  最近の接続先は `localStorage` に最大 8 件、各接続で開いたファイルは最大 10 件を非秘密情報として残す。PAT は従来どおり記憶しない。
- 朱の対象の強調表示は core の `findAnchor` と同じ規則で位置を決める。マーカーの属するブロック内の一致を優先し、
  他ブロックに唯一の一致があればそれを使い(移動扱い)、曖昧または不一致なら強調しない。
- コードの着色は Prism のトークン列を表示文字列の範囲に変換して行う。トークン長の合計が本文長と一致しない場合は着色しない。
