...
akaire は、GitHub を共有の正本とする Markdown 朱入れクライアント。
...
- ローカルは `akaire serve <cloneのパス>` 相当のランチャーを起動し、localhost の UI を開く。
...
各保存に batchId を付け、コミットメッセージの `Akaire-Batch: <UUID>` に記録する。
...
本文の指紋は、所属ブロックから akaire マーカーを除いた Markdown ソースの改行を LF に正規化して計算する。
...
AI 用の `/akaire`、CLAUDE.md、AI 実行用 GitHub Actions、モデル認証情報は本体に含めない。
